...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isModalSubmit() && interaction.customId === "pack:modal:create") {
        ...
        // Get Options
        const packName = interaction.getTextInputValue("pack:name");
        const purchaseLink = interaction.getTextInputValue("pack:link");
        const packer = interaction.getTextInputValue("pack:packer");
        const price = interaction.getTextInputValue("pack:price");
        const description = interaction.getTextInputValue("pack:description");
        ...
        const duplicate = await package.findOne({
            $or: [{ name: raw.name }, { assetId: assetId }],
        });
        if (duplicate) {
